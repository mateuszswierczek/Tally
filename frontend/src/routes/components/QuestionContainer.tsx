...
export const Route = createFileRoute('/components/QuestionContainer')({
  component: QuestionContainerPage,
})

function QuestionContainerPage() {
  return <div>Question Container Route</div>
}

interface QuestionContainerItemProps {
  i: number;
  item: {
    index: string | number;
  ...
  setMapping: (index: number) => void;
}

export function QuestionContainer({i, item, setCurrentQuestionIndex, setMapping}: QuestionContainerItemProps) {
  return <div className='h-fit min-h-15 w-full mb-5 z-1 pr-4 pl-4 flex flex-col items-center '>
            <div className='border-[0.5px]
                ...
                <button onClick={() => {setMapping(Number(i))}}>X</button>
            </div>
        </div>
}